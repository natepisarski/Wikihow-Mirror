...
//$res = DatabaseHelper::batchSelect('page', array('page_title'), array('page_namespace' => NS_MAIN, 'page_is_redirect' => 0 ), __METHOD__, array('ORDER BY'=>'page_id DESC','LIMIT' => 100));
...
function checkForConversion($page_title,$options) {
	...
	if (!$title || preg_match('@(convert|metric|calculate)@i',$title->getText()) || $title->isProtected()) {
		$convert_it = false;
	}
	...
	if ($options['miles']) {
		...
		preCheckArticle($wikitext, $miles_all, $km_all.'|eminem', $title);
			//=== MPH ===
			$units = 'mph|km/h|abbr=on';
			$regex = '(mph|miles\sper\shour)';
			list($new_wikitext, $all_tokens) = addConvertTemplate($new_wikitext, $regex, $units, $all_tokens, $title);
...
function addConvertTemplate($wikitext, $regex, $units, $tokens, $title = null) {
	global $numsWS, $conns, $negLA, $negLB;
	
	//x to y units
	if (preg_match_all("@(.?)$negLB(\b$numsWS$conns$numsWS\s?$regex\b)$negLA@iu",$wikitext,$m)) {
		list($wikitext, $tokens) = processConvertTemplates($m,$units,$wikitext,$tokens,true, $title);
	}
	
	//x units
	if (preg_match_all("@(.?)$negLB(\b$numsWS\s?$regex\b)$negLA@iu",$wikitext,$m)) {
		list($wikitext, $tokens) = processConvertTemplates($m,$units,$wikitext,$tokens,false, $title);
	}
...
function processConvertTemplates($m, $units, $wikitext, $tokens, $isSpan, $title) {
	global $negLA, $output2, $convert_it, $wgParser, $wgUser;
	$matches = $m[2];
	...
	foreach ($matches as $key => $match) {
		...
		if ($d1 !== false) {
			...
			$converted_phrase = $wgParser->preprocess($convert_tmpl,null,$options);
			$converted_phrase = str_replace('&nbsp;',' ',$converted_phrase);
			$url = ($title) ? 'http://www.wikihow.com/'.$title->getDBkey() : '';
			$output2[] = array(
				'url' => $url,
				'original_phrase' => $match,
				'conversion' => $converted_phrase,
				'converted' => $convert_it ? 'yes' : 'no');
		// }
		// else {
...
function preCheckArticle($wikitext, $this, $that, $title) {
	global $articles_skipped, $skipcheck, $output2, $convert_it;
	...
	if ($skipcheck) {
		...
		if (preg_match("@\b\d+\s?($that)\b@iu",$wikitext)) {
			$convert_it = false;
			return true;
		}
	}
...
print "url;original phrase;conversion;is_converted\n";
foreach ($output2 as $out){
	print $out['url'].";".$out['original_phrase'].";".$out['conversion'].";".$out['converted']."\n";
