...
( function ( mw, $ ) {
	...
	function insertButton( b, speedTip, tagOpen, tagClose, sampleText, imageId ) {
		...
		//XX HACK!!! XX Bebeth - couldn't figure out how to do it better
		var imageAttrs = {
			src   : b.imageFile,
			alt   : b.speedTip,
			title : b.speedTip,
		...
		if ( b.imageId != "weave_button" && b.imageId != "imageupload_button" ) {
			...
			// only add a click handler for mw core buttons. wh buttons function
			// differently
			var $image = $( '<img>' ).attr( imageAttrs ).click( function () {
				toolbar.insertTags( b.tagOpen, b.tagClose, b.sampleText );
				return false;
			} );
		} else {
			// don't add a click handler for wh buttons
			var $image = $( '<img>' ).attr( imageAttrs );
		}
