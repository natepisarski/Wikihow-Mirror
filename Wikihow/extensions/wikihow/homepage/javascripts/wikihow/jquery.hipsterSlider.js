...
/**
Liam's modifications:
    * removed code that discontinues autoplay in response to user swipe
    * changed height & width calculations to use $('.slider') instead of $(this) as an
      attempt to fix unpredictable height & width issues
    * created autoplayReset function to reset autoplay timer on swipe or button press
*/
...
(function (factory) {
	...
	$.extend(HipsterSlider.prototype, {
		...
		_init: function() {
            window.refreshSize = this.refreshSize;
		...
		_onClickPager: function(event) {
			event.preventDefault();
            this.autoplayStop(); // Liam- immediately disable autoplay
			var index = $(event.currentTarget).data('index');
			this.applyPosition(index);
			this.autoplayReset(); // Liam- reset autoplay timer
		},
        ...
        autoplayReset: function() {
            if (!this._options.autoplay) {
                ...
                window.clearTimeout(window.timer); // clear timer for current queued animation
                window.timer = window.setTimeout(proxy(this._onTimeoutAutoplay, this), duration);
            }
        },
		...
		_onTouchMove: function(event) {
			...
			var
				baseEvent = this._getTouchBaseEvent(event)
			;
			// Liam- stop autoplay feature to prevent switch during swipe:
			this.autoplayStop();
		...
		refreshSize: function() {
            ...
            // Liam- Q&D fix for page load issues on iOS
