...
class GoogleAmpAds {
	...
	private static function isStepsOnlyTest( $bucket ) {
	...
	private static function isRestrictedLowerAdsTest( $bucket ) {
		global $wgLanguageCode, $wgTitle, $wgRequest, $wgDFPAdBucket;
	...
	private static function isRestrictedLowerAdsAndStepsOnlyTest( $bucket ) {
		global $wgLanguageCode, $wgTitle, $wgRequest, $wgDFPAdBucket;
	...
	public static function insertAds() {
		...
		if ( self::isRestrictedLowerAdsAndStepsOnlyTest( $bucket ) ) {
			self::insertRestrictedLowerAdsAndStepsOnly();
		} else if ( self::isStepsOnlyTest( $bucket ) ) {
			self::insertAdsStepsOnly();
		} elseif ( self::isRestrictedLowerAdsTest( $bucket ) ) {
			self::insertAdsBottomRestricted();
		} else {
			// default case
	...
	// arg: step number
	// result: the step ad number or null if there is no ad on this step
	private static function getStepAdNumber( $i ) {
		if ( $i == 1 ) {
			return  2;
		}
		if ( $i == 4 ) {
	...
	public static function insertRestrictedLowerAdsAndStepsOnly() {
		global $wgLanguageCode, $wgTitle, $wgRequest, $wgDFPAdBucket;
		$pageId = 0;
		if ( $wgTitle ) {
		...
		// insert ads every 3 steps
		$stepsLi =  pq( ".steps_list_2 > li" );
		for ( $i = 0; $i < $stepsLi->size(); $i++ ) {
			$adNumber = self::getStepAdNumber( $i );
			if ( $adNumber == null ) {
				continue;
			}
			$adHtml = self::getAd( $adNumber, $pageId, $intlSite, $bucket );
			if ( $adHtml ) {
				pq( $stepsLi->elements[$i] )->append( $adHtml );
			}
		}

		// get number of methods
		$numMethods = pq( ".steps:not('.sample') .steps_list_2" )->size();
	...
	public static function insertAdsStepsOnly() {
		global $wgLanguageCode, $wgTitle, $wgRequest, $wgDFPAdBucket;
		$pageId = 0;
		if ( $wgTitle ) {
			$pageId = $wgTitle->getArticleID();
	...
	public static function insertAdsBottomRestricted() {
		global $wgLanguageCode, $wgTitle, $wgRequest, $wgDFPAdBucket;
		$pageId = 0;
		if ( $wgTitle ) {
			$pageId = $wgTitle->getArticleID();
