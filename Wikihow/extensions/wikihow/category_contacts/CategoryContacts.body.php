...
class CategoryContacts extends UnlistedSpecialPage {
	...
	public function execute($par) {
		...
		if ($_SERVER['HTTP_HOST'] != 'parsnip.wikiknowhow.com' && !$wgIsDevServer) {
			$wgOut->redirect('https://parsnip.wikiknowhow.com/Special:CategoryContacts');
		}
		...
		$html = EasyTemplate::html('CategoryContacts',$vars);
...
class CategoryContactMailer extends UnlistedSpecialPage {
	...
	const MW_MSG_URL = 'http://www.wikihow.com/MediaWiki:';
	const MAX_TO_SEND = 3;
	...
	public function execute($par) {
		...
		$wgCanonicalServer = 'http://www.wikihow.com';
		...
		$html = EasyTemplate::html('CategoryContactMailer',$vars);
