...
var CDN_BASE = 'https://www.wikihow.com',
	...
	$(".comdash-more").click(function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				var leader = $(".comdash-widget-" + widgetId + " .comdash-widget-leaders");
				...
				var height = $(leader).height() - 10; //not sure why we need 5, but it doesn't match up without
				var position = $(leader).position();
				var newTop = position.top - height;
...
function addUIListeners() {
	$('.comdash-pause').click(function () {
		if (!paused()) {
			$(this).html(mw.message('cd-resume-updates').text());
			paused(true);
		} else {
			$(this).html(mw.message('cd-pause-updates').text());
			paused(false);
		}
		return false;
	});
	...
	$('.comdash-settings').click(function () {
		...
		$(WH.dashboard.prefsOrdering).each(function () {
			...
			var priority = priorities[ this['wid'] ] ? mw.message('cd-current-priority').text() : '';
		...
		$.post('/Special:CommunityDashboard/customize',
			...
			function (data) {
				if (data && !data['error']) {
					...
					var error = '';
					if (!data) error = mw.message('cd-network-error').text();
