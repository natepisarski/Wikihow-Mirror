...
class CopyeditAppWidget extends DashboardWidget {
	...
	public function getLastContributor(&$dbr){
		...
		if ($sql != "")
			$res = $dbr->select('logging', array('log_user','log_timestamp'), array('log_type' => 'EF_copyedi', $sql), 'CopyeditAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
		else
			$res = $dbr->select('logging', array('log_user','log_timestamp'), array('log_type' => 'EF_copyedi'), 'CopyeditAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
	...
	public function getTopContributor(&$dbr){
		...
		// Note log_type == EF_copyedi, not EF_copyediT.  wiki log types can only be 10 chars, so truncating
		if ($sql != "")
			$res = $dbr->select('logging', array('log_user', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'EF_copyedi', 'log_timestamp >= "' . $starttimestamp . '"', $sql), 'CopyeditAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		else
			$res = $dbr->select('logging', array('log_user', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'EF_copyedi', 'log_timestamp >= "' . $starttimestamp . '"'), 'CopyeditAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		$row = $dbr->fetchObject($res);
		$res->free();
