...
class NfdAppWidget extends DashboardWidget {
	...
	public function getLastContributor(&$dbr){
		...
		if ($sql != "")
			$res = $dbr->select('logging', array('log_user','log_timestamp'), array('log_type' => 'nfd', 'log_action' => 'vote', $sql), 'NfdAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
		else
			$res = $dbr->select('logging', array('log_user','log_timestamp'), array('log_type' => 'nfd', 'log_action' => 'vote'), 'NfdAppWidget::getLastContributor', array("ORDER BY"=>"log_timestamp DESC", "LIMIT"=>1));
	...
	public function getTopContributor(&$dbr){
		...
		if ($sql != "")
			$res = $dbr->select('logging', array('log_user', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'nfd', 'log_action' => 'vote', 'log_timestamp >= "' . $starttimestamp . '"', $sql), 'NfdAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		else
			$res = $dbr->select('logging', array('log_user', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'nfd', 'log_action' => 'vote', 'log_timestamp >= "' . $starttimestamp . '"'), 'NfdAppWidget::getTopContributor', array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		$row = $dbr->fetchObject($res);
		$res->free();
