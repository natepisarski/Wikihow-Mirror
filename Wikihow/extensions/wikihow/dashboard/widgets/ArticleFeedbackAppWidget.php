...
class ArticleFeedbackAppWidget extends DashboardWidget {
	...
	public function getLastContributor(&$dbr){
		$res = $dbr->select('logging', array('*'), array('log_type' => "article_feedback_tool"), __FUNCTION__, array("ORDER BY"=>"log_timestamp DESC", "LIMIT" => 1));
	...
	public function getTopContributor(&$dbr){
		...
		$res = $dbr->select('logging', array('*', 'count(*) as C', 'MAX(log_timestamp) as log_recent'), array('log_type' => 'article_feedback_tool', 'log_timestamp >= "' . $starttimestamp . '"'), __FUNCTION__, array("GROUP BY" => 'log_user', "ORDER BY"=>"C DESC", "LIMIT"=>1));
		$row = $dbr->fetchObject($res);
		$res->free();
