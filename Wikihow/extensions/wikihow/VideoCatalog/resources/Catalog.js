 ...
 *
 * @class
 * @param {Object} data Catalog data, use the result of api.php?action=summary_videos when loading
 *   asynchronously, or embed the JSON encoded result of ApiSummaryVideos::query() in the HTML
 *   output if loading synchronously
 * @param {string} [store='video-catalog-watched'] LocalStorage key for storing the watched list
 */
WH.VideoCatalog = function Catalog( options ) {

	/* Default Options */

	options.videos = options.videos || [];
	options.store = options.store || 'video-catalog-watched';
	...
	var catalog = this;
	var watched = JSON.parse( localStorage.getItem( options.store ) || '{}' );
	// Insert data from server
	catalog.videos.insert( options.videos );
...
WH.VideoCatalog.prototype.persist = function () {
...
WH.VideoCatalog.prototype.watchVideo = function ( video ) {
	var catalog = this,
...
WH.VideoCatalog.prototype.rankCategory = function ( category ) {
