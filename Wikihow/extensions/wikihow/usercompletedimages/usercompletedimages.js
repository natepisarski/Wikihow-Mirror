( function ( mw, $ ) {
	...
	WH.usercompletedimages = {
		...
		loadHtml: function() {
			var url = '/Special:Usercompletedimages?action=get_mobile_html&article_id=' + mw.config.get('wgArticleId');

			$.getJSON( url, $.proxy(function(data) {
					if (data && data.html)
						this.insertHtml(data.html)
					else
						this.removePlaceholderSection();
				}, this)
			);
		},

		insertHtml: function( html ) {
			$('#uci_section').html(html);
			this.addHandlers();
			mw.loader.load('mobile.wikihow.uci_after');
		},

		removePlaceholderSection: function() {
			$('#uci_section').remove();
		},
		...
		init : function() {
			if (WH.isMobileDomain) {
				...
				this.setupFlagging($('.uci_flaggable'));
				this.addHandlers();
			}
