		...
		getPage: function( title, endpoint, leadOnly ) {
			...
			if ( !this.cache[title] ) {
				...
				this.get( {
					...
					if ( resp.error || !resp.mobileviewwikihow.sections ) {
						page.reject( resp );
					// FIXME: [LQT] remove when liquid threads is dead (see Bug 51586)
					} else if ( resp.mobileviewwikihow.hasOwnProperty( 'liquidthreads' ) ) {
						page.reject( { error: { code: 'lqt' } } );
					} else {
						mv = resp.mobileviewwikihow;
						...
						resolveObj = {
							...
							articleText: mv.text /* XXCHANGED: BEBETH */
						};
