			...
			if ( isset( $data['parse']['text'] ) && $params['mobileformat'] ) {
				...
				if ( $mobileformat === 'html' || $mobileformat === 'wml' ) {
					...
					if(isset($params['sectionId'])) {
						...
						$newText = $wgParser->replaceSection($revision->getText(), $params['sectionId'], $params['text']);

						global $wgUser, $wgContLang;
						$popts = ParserOptions::newFromUserAndLang( $wgUser, $wgContLang );

						$contentHandler = new WikitextContentHandler();
						$content = $contentHandler->unserializeContent($newText);
						...
						$outputPage = $module->getContext()->getOutput();
						...
						$text = WikihowMobileTools::processDom($text, $module->getContext()->getSkin());

						ApiResult::setContent( $arr, $text );
						$data['parse']['sectionId'] = $params['sectionId'];
					}
					else {
