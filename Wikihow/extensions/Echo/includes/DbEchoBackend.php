...
class MWDbEchoBackend extends MWEchoBackend {
	...
	public function createNotification( $row ) {
		...
		$dbw->onTransactionIdle(
			function() use ( $dbw, $row, $fname ) {
				...
				if ( $row['notification_bundle_display_hash'] ) {
					$dbw->update(
						...
						array(
							...
							'notification_anon_ip' => ! $row['notification_user'] ? $row['notification_anon_ip'] : '',
				...
				//XXCHANGEDXX - allow anons [sc|rs]
				$user = ! $row['notification_user']
					? User::newFromName( $row['notification_anon_ip'], false )
					: User::newFromId( $row['notification_user'] );
	...
	public function loadNotifications( $user, $limit, $continue, $outputFormat = 'web' ) {
		...
		//XXCHANGEDXX - allow anons [sc|rs]
		if ( $user->isAnon() ) {
			$conds += array( 'notification_anon_ip' => $user->getName() );
		}
	...
	public function getRawBundleData( $user, $bundleHash, $type = 'web', $order = 'DESC', $limit = 250 ) {
		...
		if ( $type == 'web' ) {
			$res = $dbr->select(
				...
				+ ($user->isAnon() ? array( 'notification_anon_ip' => $user->getName() ) : array()),
				__METHOD__,
