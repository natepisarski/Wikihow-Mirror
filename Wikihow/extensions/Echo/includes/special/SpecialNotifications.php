...
class SpecialNotifications extends SpecialPage {
	...
	public function execute( $par ) {
		...
		// Wikihow: mark all messages as read any time users visit
		// Special:Notifications. Per Anna, simplifying page.
		$notifUser = MWEchoNotifUser::newFromUser( $user );
		$ret = $notifUser->markAllRead();
		...
		// Wikihow: remove the "Mark all read" button, per Anna.
		if ( false &&  $anyUnread ) {
		...
		foreach ( $notifArray as $section => $data ) {
			...
			// Wikihow: remove Mark all group as unread button, per Anna
			if ( false && $data[ 'unread' ] !== [] ) {
