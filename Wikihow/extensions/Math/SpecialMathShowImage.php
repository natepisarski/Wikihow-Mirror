...
class SpecialMathShowImage extends SpecialPage {
	...
	function setHeaders( $success = true ) {
		...
		if ( $success && !( $this->noRender ) ) {
			$request->response()->header( 'Cache-Control: public, s-maxage=604800, max-age=604800' ); // 1 week (server) 1 week (client)
 ...
 preserveAspectRatio="xMidYMid meet" >' .
	...
	// Added by Reuben, wikiHow, Sept 3, 2015. We need this method so this the
	// special page won't redirect on the mobile domain.
