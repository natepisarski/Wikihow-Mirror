...
class Minify {
    ...
    public static function serve($controller, $options = array())
    {
        ...
        // Default last modified time is now
        $fileLastModified = 0;
        ...
        if (null !== self::$_cache) {
            ...
            if ($cacheIsReady) {
                ...
                $fileLastModified = self::$_cache->getTime($fullCacheId);
            } else {
        ...
        if (self::$_options['encodeOutput']) {
            //XXCHANGED: X-SSL-Client header (see Fastly config for how this is defined)
            // added to Vary list for extensions/min output. This is important so that
            // https content is recognized as different from http content within the cache.
            $headers['Vary'] = 'Accept-Encoding, X-SSL-Client';
        }
        ...
        if (! self::$_options['quiet']) {
            ...
            $fileLastModified = $fileLastModified ? $fileLastModified : time();
            header('Cache-Control: max-age=' . $thirtyDays);
            header('Last-Modified: ' . gmdate('D, d M Y H:i:s', $fileLastModified) . ' GMT');
            header('Expires: ' . gmdate('D, d M Y H:i:s', time() + $thirtyDays) . ' GMT');
            if ($cacheIsReady) {
    ...
    protected static function _combineMinify()
    {
        ...
        //XXCHANGED
        if (self::$_options['contentType'] === self::TYPE_CSS) {
            $content = self::_modCssCdn($content);
        }
        return $content;
    }
     ...
     * better browser downloading parallelization, or remove it for
     * SSL sites to keep the browser lock icon.
     * @param string $css the bundle of CSS to modify
     */
    protected static function _modCssCdn($content) {
        // A big hack so we don't have to recreate wfGetPad() or wfRewriteCSS().
