...
class SpecialRenameuser extends SpecialPage {
	...
	public function execute( $par ) {
		...
		// Wikihow: underscore is valid in our user names
		//was: $oldnamePar = trim( str_replace( '_', ' ', $request->getText( 'oldusername', $usernames[0] ) ) );
		$oldnamePar = trim( $request->getText( 'oldusername', $usernames[0] ) );
		$oldusername = Title::makeTitle( NS_USER, $oldnamePar );
		$newnamePar = isset( $usernames[1] ) ? $usernames[1] : null;
		//was: $newnamePar = trim( str_replace( '_', ' ', $request->getText( 'newusername', $newnamePar ) ) );
		$newnamePar = trim( $request->getText( 'newusername', $newnamePar ) );
		...
		// Wikihow/Scott: this was having an issue with underscores
		//was: if ( $oldusername->getText() !== $wgContLang->ucfirst( $oldusername->getText() ) ) {
		...
		if ( $uid === 0 ) {
			// Wikihow: use THIS username
			$out->wrapWikiMsg( "<div class=\"errorbox\">$1</div>",
				[ 'renameusererrordoesnotexist', $olduser->getName() ] );
		...
		if ( !Hooks::run(
			'RenameUserAbort',
			[ $uid, $olduser->getName(), $newusername->getText() ]
		) ) {
			return;
		}
		...
		$rename = new RenameuserSQL(
			$olduser->getName(), // Wikihow: use THIS username
			$newusername->getText(),
			$uid,
			$this->getUser(),
			[ 'reason' => $reason ]
		...
		if ( $request->getCheck( 'movepages' ) && $user->isAllowed( 'move' ) ) {
			...
			foreach ( $pages as $row ) {
				...
				if ( $newPage->exists() && !$validMoveStatus->isOK() ) {
					...
					$logReason = $this->msg(
						'renameuser-move-log', $olduser->getName(), $newusername->getText()
		...
		$out->wrapWikiMsg( "<div class=\"successbox\">$1</div><br style=\"clear:both\" />",
			[ 'renameusersuccess', $olduser->getName(), $newusername->getText() ] );
	}
