...
class AbuseFilter {
	...
	public static function filterAction(
		...
		list($mapper, $destUser) = EditMapper\EditMapperHooks::getActiveMapper($title, $user, $isNew);
		...
		if ( count( $matched_filters ) == 0 ) {
			...
			if ( $isForEdit && $cacheData ) {
				//mwup: The timing of this deferred update for self::addLogEntries is causing some problems
				//because it's happening after we try and put the revision id into the table in the
				//pageSaveComplete hook. So no longer making it a deferred update
		...
		// wikiHow (Alberto, 2017-01-24) - Ticket #1834
		if ($mapper) {
			$mapper->undoMapping(WikiPage::factory($title), $user);
		}
		// End of wikiHow
