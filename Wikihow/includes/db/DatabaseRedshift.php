<?php
/**
 * This is the Redshift database abstraction layer.
 *
...
/**
	...
	* This was copied almost wholesale from Database Postgres. We commented out
	* a few lines - search for 'UNSUPPORTED FOR REDSHIFT'. The rationale is that we
	* wanted a mediawiki-compatible db adapter, and redshift _almost_ speaks a modern
	* postgres dialect. Almost.
	* I tried subclassing this instead, but kept running into really weird errors
	* and decided just to copy the file.
	* If you run into problems when upgrading MW, you should probably just replace
	* this with a new version of DatabasePostgres, s/postgres/redshift/gi, and then
	* comment out the same blocks as in this file (again, search for 'UNSUPPORTED').
**/
/**
	* DIRE WARNING
	* This has not been extensively tested. Reads work! Writes PROBABLY WILL NOT.
	* If you find yourself trying to write to redshift from MW, please test thoroughly.
**/

class Redshift implements Field {
 ...
 * @ingroup Database
 */
class RedshiftTransactionState {
 ...
 * @ingroup Database
 */
class DatabaseRedshift extends DatabaseBase {
	/** @var resource */
	...
	/** @var string Connect string to open a redshiftQL connection */
	private $connectString;

	/** @var RedshiftTransactionState */
	...
	function open( $server, $user, $password, $dbName ) {
		# Test for Redshift support, to avoid suppressed fatal error
		if ( !function_exists( 'pg_connect' ) ) {
			throw new DBConnectionError(
				$this,
				"Redshift functions missing, have you compiled PHP with the --with-pgsql\n" .
				"option? (Note: if you recently installed PHP, you may need to restart your\n" .
				"webserver and database)\n"
			);
		}

		global $wgRedshiftPort;
		...
		$this->mOpened = true;
		$this->mTransactionState = new RedshiftTransactionState( $this->mConn );
	...
	/**
	 * Redshift doesn't support selectDB in the same way MySQL does. So if the
	...
	public function doQuery( $sql ) {
		...
		if ( pg_send_query( $this->mConn, $sql ) === false ) {
			throw new DBUnexpectedError( $this, "Unable to post new query to redshiftQL\n" );
		}
	...
	function freeResult( $res ) {
		...
		wfRestoreWarnings();
		if ( !$ok ) {
			throw new DBUnexpectedError( $this, "Unable to free Redshift result\n" );
		}
	}
...
__INDEXATTR__;
	 ...
	 *
	 * In Redshift when using FOR UPDATE, only the main table and tables that are inner joined
	 ...
	 * for multi-row insert (Redshift version 8.2 and above only).
	 *
	...
	function insert( $table, $args, $fname = __METHOD__, $options = array() ) {
		...
		if ( in_array( 'IGNORE', $options ) ) {
			$savepoint = new SavepointRedshift( $this, 'mw' );
	...
	function timestamp( $ts = 0 ) {
		return wfTimestamp( TS_redshift, $ts );
	}
	...
	public function getSoftwareLink() {
		return '[{{int:version-db-redshift-url}} redshiftQL]';
	}
	...
	function getSearchPath() {
		...
		/* redshiftQL returns SHOW values as strings */
...
SQL;
	...
	function fieldInfo( $table, $field ) {
		return RedshiftField::fromText( $this, $table, $field );
	}

	/**
	 * pg_field_type() wrapper
	 * @param ResultWrapper|resource $res ResultWrapper or redshiftQL query result resource
	...
	/**
	 * Redshift specific version of replaceVars.
	 ...
	 * See http://www.redshiftql.org/docs/8.2/static/functions-admin.html#FUNCTIONS-ADVISORY-LOCKS
	 *
	...
	/**
	 * See http://www.redshiftql.org/docs/8.2/static/functions-admin.html#FUNCTIONS-ADVISORY-LOCKSFROM
	 * PG DOCS: http://www.redshiftql.org/docs/8.2/static/functions-admin.html#FUNCTIONS-ADVISORY-LOCKS
	 * @param string $lockName
	 * @param string $method
	 * @return bool
	 */
