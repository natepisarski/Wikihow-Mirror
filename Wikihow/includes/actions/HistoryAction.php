...
class HistoryPager extends ReverseChronologicalPager {
	...
	function getStartBody() {
		...
		$this->buttons = '<div>';
		//XXCHANGEDXX Bebeth: added class to submit button
		$this->buttons .= $this->submitButton( $this->msg( 'compareselectedversions' )->text(),
			array( 'class' => 'button secondary historysubmit mw-history-compareselectedversions-button' )
	...
	function historyLine( $row, $next, $notificationtimestamp = false,
		...
		// XXCHANGED Reuben 1/23/2014: show last patrolled revision to staff
		if( $this->historyPage->mShowGoodRev ) {
			if( $this->historyPage->mGoodRev ) {
				if( $rev->getId() == $this->historyPage->mGoodRev ) {
					$s .= ' <b>GR</b>';
				}
			} elseif( $latest ) {
				$s .= ' <b>GR</b>';
