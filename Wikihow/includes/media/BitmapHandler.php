...
class BitmapHandler extends TransformationalImageHandler {
	...
	protected function transformImageMagick( $image, $params ) {
		...
		// ARG - added code to deal with cropping of images)
		...
		$cmd = wfEscapeShellArg( ...array_merge(
			...
			// a pixel.
			// ARG changed how the thumbmail param is added and added crop param
			$thumbnail,
			$crop,
		...
		$retval = 0;
		if (Hooks::run("ImageConvert", array($params))) {
			$err = wfShellExecWithStderr( $cmd, $retval, $env );
			Hooks::run("ImageConvertComplete", array($params));
		}
