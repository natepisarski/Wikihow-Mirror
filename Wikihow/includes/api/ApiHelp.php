		...
		foreach ( $modules as $module ) {
			...
			if ( $examples ) {
				...
				foreach ( $examples as $qs => $msg ) {
					...
					// Wikihow: this page ApiSandbox didn't work, and the PMs don't want
					// it, so changing this link to poin elsewhere
					#$sandbox = SpecialPage::getTitleFor( 'ApiSandbox' )->getLocalURL() . '#' . $qs;
					$sandbox = 'https://en.wikipedia.org/wiki/Special:ApiSandbox#' . $qs;
