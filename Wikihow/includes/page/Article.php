...
class Article implements Page {
	...
	public function view() {
		...
		// Wikihow/Reuben, 3/19/2018: changed this, for security, as recommended by
		// Dareboost report. Note: this change makes article pages have the
		...
		if ( $outputPage->isPrintable() ) {
			...
			// Wikihow/Scott: always show links for mobile
		...
		while ( !$outputDone && ++$pass ) {
			switch ( $pass ) {
					...
					if ( $useParserCache ) {
						...
						if ( $this->mParserOutput !== false ) {
							...
							if ( $cachedTimestamp !== null ) {
								...
								// Wikihow/Aaron: we use this field to get the dateModified for the schema output
				...
				case 3:
					...
					// Wikihow/Aaron: added this for the case the lastest good revision is not the most recent revision
					if ( $this->mRevision ) {
	...
	protected function showDiffPage() {
		...
		if ( !$rev ) {
			// Wikihow/JRS 02/25/14: Set a 404 status code for this error since google doesn't like soft 404s
	...
	public function getRobotPolicy( $action, ParserOutput $pOutput = null ) {
		...
		if ( $this->mPage->getId() === 0 || $this->getOldID() ) {
			...
			// Wikihow/JRS: Commented out the following lines since GoodRevision handles this now
			//return [
			//	'index' => 'noindex',
			//	'follow' => 'nofollow'
			//];
	...
	public function showMissingArticle() {
		...
		if ( !$oldid && $title->getNamespace() === NS_MEDIAWIKI && $title->hasSourceText() ) {
			...
			// Wikihow: added this hook to be able to change the html output in this case
			Hooks::run( 'BeforeDisplayNoArticleInterface', [ $this, $dir, $lang, $text, &$html ] );
			$outputPage->addWikiTextAsInterface( $html );
		}

		// Wikihow: added this hook
		Hooks::run( 'AfterDisplayNoArticleText', [ $this ] );
	}
	...
	public static function getRedirectHeaderHtml( Language $lang, $target, $forceKnown = false ) {
		...
		//BS: Upgrade lost the redirect arrow and we wanted it back
		global $wgStylePath;
		$imageUrl = $wgStylePath . '/common/images/redirect' . $lang->getDir() . '.png';
	...
	public function confirmDelete( $reason ) {
		...
		$fields[] = new OOUI\FieldLayout(
			new OOUI\TextInputWidget( [
				...
				'value' => $reason,
				'autofocus' => true,
				'class' => 'input_med', // Wikihow: added this class
			] ),
			[
		...
		$fields[] = new OOUI\FieldLayout(
			new OOUI\ButtonInputWidget( [
				...
				'type' => 'submit',
				'class' => 'button primary', // Wikihow: added classes
			] ),
			[
				'align' => 'top',
			]
	...
	//
	// Wikihow: Mediawiki class ends and everything after this point was added
	// by Wikihow for convenience.
	//
	...
	public static function getSection($text, $section) {
		global $wgParser;
		return $wgParser->getSection($text, $section);
	}

	public function updateArticle($text, $summary, $minor, $watchthis, $forceBot = false, $sectionanchor = '' ) {
	...
	public function insertNewArticle($text, $summary, $minor, $watchthis, $forceBot = false, $sectionanchor = '' ) {
		wfDeprecated( __METHOD__ );
