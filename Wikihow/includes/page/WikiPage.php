...
class WikiPage implements Page, IDBAccessObject {
	...
	/**
	 * Wikihow: we keep track of page views and use this data, so
	 * it's convenient and efficient to grab it at the same time.
	 * See uses of this class member and also the method we re-added
	 * to this class called getCount().
	 */
	...
	public static function selectFields() {
		...
		// Wikihow: load page_counter column too
		$fields[] = 'page_counter';
	...
	public function loadFromRow( $data, $from ) {
		...
		if ( $data ) {
			...
			// Wikihow: added this field
			$this->mCounter = (int)$data->page_counter;
		} else {
	...
	public function commitRollback( $fromP, $summary, $bot,
		...
		// Change by Gershon to also replace - with space in username and allow un-replaced '_' in username to rollback
		$currentEditorForPublic = $current->getUser( RevisionRecord::FOR_PUBLIC );
		$legacyCurrent = new Revision( $current );
		$from = preg_replace( '/[_-]/', ' ', $fromP );
