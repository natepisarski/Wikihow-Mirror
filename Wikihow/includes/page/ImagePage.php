...
class ImagePage extends Article {
	...
	protected function openShowImage() {
		...
		$maxWidth = $max[0];
		// Wikihow/Bebeth: For fixed width new layout. These numbers don't entirely
		// make sense, but don't want to make new thumbnail sizes.
		if ($maxWidth > 679) {
			$maxWidth = 670;
		}
		$maxHeight = $max[1];
		...
		if ( $this->displayImg->exists() ) {
			...
			if ( $this->displayImg->allowInlineDisplay() ) {
				...
				// Wikihow/JRS: Disable this message for our image pages
				$msgsmall = '';
				...
				if ( $thumbnail ) {
					...
					// Wikihow: add additional class name to this class
					$out->addHTML( '<div class="fullImageLink minor_section" id="file">' .
