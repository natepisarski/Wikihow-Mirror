...
class DatabaseMysqli extends DatabaseMysqlBase {
	...
	protected function doQuery( $sql ) {
		...
		// Wikihow merge note: IGNORE THESE CHANGES WHILE MERGING. DON'T MERGE THEM.
		// THIS IS A BIG HACK, only for the old MW code. It detects when one of
		// these tables aren't found, and retries the query on a differently named
		// version of the table. Suitable only for use in production for a few hours
		// while we are transitioning to new MW version code.
		if ($ret === false && $this->lastErrno() == 1146) { // bad result and missing table was cause
			// these tables are about to be dropped, and are renamed to *_tmp during transition time
			$tables = ['msg_resource_links', 'msg_resource', 'transcache', 'valid_tag', 'tag_summary'];
			if ( preg_match('@(FROM|INTO) `(' . join('|', $tables) . ')`@', $sql) ) {
				$sql = preg_replace('@(FROM|INTO) `(' . join('|', $tables) . ')`@', '$1 `$2_tmp`', $sql);
				// Re-run query with updated tmp table name
