...
class OutputPage extends ContextSource {
	...
	// Wikihow/Aaron: added to fix a bug where revisionTimestamp is incorrect
	...
	// Wikihow/JRS 12/11/13 Adding a debug variable for robot policy
	 ...
	 * @note Wikihow/Reuben: added so that we can defer ResourceLoader startup on page
	 */
	private $mDeferHeadScripts = '';

	/**
	 * @var string Wikihow/Reuben: added code for debugging. See references to
	 *   this variable below for details.
	 */
	...
	/**
	 * Wikihow: needed this for our AMP implementation to work.
	 */
	public function clearModuleStyles( ) {
		$this->mModuleScripts = [];
	}
	...
	public function addCategoryLinks( array $categories ) {
		...
		if ( Hooks::run(
			...
			foreach ( $categories as $category => $type ) {
				...
				// Wikihow/Alberto, 2018-04-01: added so that we only
				// link to indexable category pages.
	...
	public function sendCacheControl() {
		...
		if ( $this->mEnableClientCache ) {
			// Wikihow/Reuben: added live output header to debug why CDN caching isn't happening
			$noCache = [];
			if ( !$config->get( 'UseSquid' ) ) { $noCache[] = 'wg'; }
			if ( $response->hasCookies() ) { $noCache[] = 'ck'; }
			if ( SessionManager::getGlobalSession()->isPersistent() ) { $noCache[] = 'ss'; }
			if ( $this->isPrintable() ) { $noCache[] = 'pr'; }
			if ( $this->mCdnMaxage == 0 ) { $noCache[] = 'ma'; }
			if ( $this->haveCacheVaryCookies() ) { $noCache[] = 'va'; }
			...
			if (
				...
				// Wikihow/Reuben: added to see $noCache as a user
	...
	public function output( $return = false ) {
		...
		if ( $this->mArticleBodyOnly ) {
			...
			// Wikihow/Reuben, 11/19/2014: We need to send cache control headers early
	...
	public function headElement( Skin $sk, $includeStyle = true ) {
		...
		if ( !Html::isXmlMimeType( $this->getConfig()->get( 'MimeType' ) ) ) {
			...
			// Wikihow/Aaron: changed utf-8 to be lowercase for amp validation
	...
	public function getHeadLinksArray() {
		...
		// JRS 12/11/13 explicitly set robot policy
	...
	public function setRobotPolicyCustom($policyString, $debugReason) {
		$this->mRobotPolicyDebug = $debugReason;
		$arr = explode(',', $policyString);
		$this->setRobotPolicy(array('index' => $arr[0], 'follow' => $arr[1]));
	}
}
