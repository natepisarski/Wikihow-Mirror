...
class PageUpdater {
	...
	private function doModify( CommentStoreComment $summary, User $user, $flags ) {
		...
		if ( $changed ) {
			...
			// Wikihow: only count certain edits (ie, not Kudos, User, User_talk etc)
			if ( $user->shouldCountEdit( $wikiPage->getTitle() ) ) {
				$user->incEditCount();
			}
	...
	private function doCreate( CommentStoreComment $summary, User $user, $flags ) {
		...
		// Wikihow: always patrol new articles. In our system, NAB is in place for new
		// articles, and no reason to do NAB + RC (per Anna, July 2019).
		if ( $this->getTitle()->inNamespace( NS_MAIN ) ) {
			$this->setRcPatrolStatus( RecentChange::PRC_PATROLLED );
		}
