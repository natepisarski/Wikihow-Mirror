...
class UserMailer {
	...
	public static function send( $to, $from, $subject, $body, $options = [] ) {
		...
		// Wikihow: filter email address from the To: list that we know to
		// bounce at SendGrid. It's important to filter these because
		// trying to send many different emails to bouncing addresses lowers
		// your domain's email reputation.
		Hooks::run( 'FilterOutBouncingEmails', [ &$to ] );
	...
	protected static function sendInternal(
		...
		// Wikihow: added shortcut for X-SMTPAPI header, which is used by sendgrid
		if ( $options['category'] ?? '' ) {
			$options['headers']['X-SMTPAPI'] = json_encode( ['category' => $options['category']] );
		}
		...
		// Wikihow: this -f setting, combined with what we like to use as our standard
		//   From: wikiHow Team <support@wikihow.com
		// header, $extraParams breaks our mail() call. Blanking this out fixes.
		$extraParams = '';
		...
		if ( is_array( $wgSMTP ) ) {
			...
			try {
				global $wgDisableScriptEmails, $wgIsDevServer;
				foreach ( $to as $recip ) {
					// Wikihow 2015: added ability to disable all email sending from a script
					if ( $wgDisableScriptEmails ) continue;

					// Wikihow: only send email if we're in a production environment OR
					// the recipient is someone@wikihow.com. We do this so that non-staff
					// do not get spammed from our dev site. We also have logic to look at
					// a custom list of recipients who can receive emails from dev for testing.
					...
					if ( !$wgIsDevServer || preg_match('/@wikihow\.com>?$/', $recip) || $authorizedDevEmail ) {
						...
						$sent = true; // fake that this sent, for dev
					}
