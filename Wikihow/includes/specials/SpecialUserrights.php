...
class UserrightsPage extends SpecialPage {
	...
	private function groupCheckboxes( $usergroups, $user ) {
		...
		// wikiHow staff? [sc - 1/16/2014]
		global $wgUser;
		$isStaff = in_array('staff', $wgUser->getGroups());
		$staff_only_groups = array('staff_widget', 'translator');
		...
		foreach ( $columns as $column ) {
			...
			foreach ( $column as $group => $checkbox ) {
				// ancient Chinese secret checkboxes for staff users [sc - 1/16/2014]
				if (!$isStaff && in_array($group, $staff_only_groups) && !$checkbox['set']) continue;
