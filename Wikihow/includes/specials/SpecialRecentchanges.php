...
class SpecialRecentChanges extends ChangesListSpecialPage {
	...
	protected function doMainQuery( $tables, $fields, $conds, $query_options,
		...
		if ( !$this->runMainQueryHook( $tables, $fields, $conds, $query_options,
				$join_conds, $opts, $reverse )
		) {
			return false;
		}
		...
		//mwup: adding this code here directly instead of in the hook like before.
		//the old hook totally changed and doesn't really support what we need.
		global $wgRequest;
		...
		$orderByAndLimit = [
			'ORDER BY' => 'rc_timestamp ' . $queryOrder,
			'LIMIT' => $opts['limit']
		];
	...
	public function doHeader( $opts, $numRows ) {
		...
		$submit = ' ' . Xml::submitButton(
			$this->msg( 'recentchanges-submit' )->text(),
	...
	function setTopText( FormOptions $opts ) {
		...
		if ( !$message->isDisabled() ) {
			...
			$topLinksAttributes = [ 'class' => 'mw-recentchanges-toplinks minor_text' ];
			...
			$this->getOutput()->addHTML(
				Html::rawElement( 'div', $topLinksAttributes, $content ) . '<br>'
			);
		}
	}
