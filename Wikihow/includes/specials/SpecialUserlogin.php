...
class LoginForm extends SpecialPage {
	...
	function addNewAccount() {
		...
		if ( !$status->isGood() ) {
			$error = $status->getMessage();
			//$this->mainLoginForm( $error->toString() );
			//XXCHANGEDXX - adding Gershon's verbose error hack in via a function [sc]
			$errorlist = WikihowLogin::generateErrorList($status->getErrorsArray());
			$this->mainLoginForm( $error->toString(), 'error', $errorlist);
			return false;
		}
		...
		if ( $this->getUser()->isAnon() ) {
			...
			if ( class_exists( 'GDPR' ) ) {
				$gdpr = GDPR::isGDPRRequest();
				if ( $gdpr ) {
					$u->setOption( 'gdpr_signup', true );
				}
			}
	...
	function processLogin() {
		...
		switch ( $this->authenticateUserData() ) {
			...
			case self::ABORTED:
				...
				break;
			case self::NO_EMAIL:
				$this->mainLoginForm( $this->msg( 'noemail_login')->text(), 'error', array('username') );
				break;
			case self::MULTIPLE_EMAILS:
				$this->mainLoginForm( $this->msg( 'multipleemails_login')->text(), 'error', array('username') ) ;
				break;
			default:
