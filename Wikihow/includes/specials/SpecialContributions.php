...
class SpecialContributions extends IncludableSpecialPage {
	...
	public function execute( $par ) {
		...
		if ( wfRunHooks( 'SpecialContributionsBeforeMainOutput', array( $id, $userObj, $this ) ) ) {
			if ( !$this->including() ) {
				$out->addHTML( $this->getForm() );
				// Scott, 1.21 upgrade: added because we don't use the addSubtitle() 
				// (see line 91)
				$out->addHTML("<div style='font-size:95%; margin-bottom:10px;'>" . $this->contributionsSub( $userObj ) . "</div>" );
			}
	...
	protected function getForm() {
		...
		$input = Html::input(
			...
			$this->opts['target'],
			'text',
			array( 'size' => '40', 'required' => '', 'class' => 'input_med' ) +
...
//		);
		...
		$dateSelectionAndSubmit = Xml::tags( 'td', array( 'colspan' => 2 ),
			...
			// JRS 12/5/13 Style changes to support redesign
			Xml::submitButton(
				$this->msg( 'sp-contributions-submit' )->text(),
				array( 'class' => 'mw-submit button primary', 'style' => 'float:right;margin:5px;' )
			)
		);
