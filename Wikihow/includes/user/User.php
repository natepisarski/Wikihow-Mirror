...
class User implements IDBAccessObject, UserIdentity {
	...
	// Wikihow: added for social network features
	private $mLoadedFacebook = false,
		$mIsFacebook = false,
		$mLoadedGPlus = false,
		$mIsGPlus = false,
		$mLoadedCivic = false,
	...
	public static function isCreatableName( $name ) {
		...
		if ( Hooks::run( 'UserValidateName', [ $name, &$result ] ) ) {
	...
	public function getOption( $oname, $defaultOverride = null, $ignoreHidden = false ) {
		...
		Hooks::run( 'getUserPref', [ $oname, &$this->mOptions ] );
	...
	public function initEditCountInternal() {
		...
		// Wikihow/Reuben, circa 2014: Change this user edit count to exclude any
		// edits from certain namespaces.
		$dbr = wfGetDB( DB_REPLICA );
		$actorWhere = ActorMigration::newMigration()->getWhere( $dbr, 'rev_user', $this );
	...
	// Wikihow/Reuben, circa 2014: We only want to increment the user edit
	// count if making a non-NS_USER/NS_USER_TALK/KUDOS edit, so we provide
	// this method, which is called by PageUpdater
	public function shouldCountEdit( $title ) {
		global $wgIgnoreNamespacesForEditCount;
		if ( is_array( $wgIgnoreNamespacesForEditCount )
			&& $title->inNamespaces( $wgIgnoreNamespacesForEditCount )
		) {
			return false;
		} else {
			return true;
	...
	public function getNewKudos() {
		...
		if ( $this->mNewKudos == -1 ) {
			...
			$dbr = wfGetDB( DB_REPLICA );
			if ( $this->mId ) {
				$res = $dbr->select( 'user_newkudos', 1, [ 'user_id' => $this->mId ], __METHOD__ );
				...
				$key = self::newKudosIPcacheKey( $this->mName );
				$newkudos = $wgMemc->get( $key );
				if ( ! is_integer( $newkudos ) ) {
					$res = $dbr->select( 'user_newkudos', 1, [ 'user_ip' => $this->mName ], __METHOD__ );
	...
	private static function notificationCacheKey( $id ) {
		return wfMemcKey( 'notification_box', $id );
	}

	private static function updateNewKudos( $field, $id ) {
		global $wgMemc;
		$memkey = self::notificationCacheKey( $id );
	...
	private static function deleteNewKudos( $field, $id ) {
		global $wgMemc;
		$memkey = self::notificationCacheKey( $id );
		$wgMemc->delete($memkey);
	...
	private static function newKudosIPcacheKey( $val ) {
		return wfMemcKey( 'newkudos:ip', $val );
	}
	...
	public function setNewKudos( $val ) {
		...
		if ( $changed ) {
			if ( $this->isAnon() ) {
				...
				global $wgDBname, $wgMemc;
				$key = self::newKudosIPcacheKey( $val );
