...
class DefaultPreferencesFactory implements PreferencesFactory {
	...
	protected function profilePreferences(
		...
		if ( $wgIsDevServer && $this->config->get( 'SecureLogin' ) && $canIPUseHTTPS ) {
		...
		Hooks::run('CreateEmailPreferences', array(&$user, &$defaultPreferences));
	...
	protected function saveFormData( $formData, HTMLForm $form, array $formDescriptor ) {
		...
		if ( !in_array( 'realname', $hiddenPrefs )
			...
			// mwupgrade2019: If the real name doesn't pass validation just set to an empty string
			// In the future we should send back a validation error if the real names don't pass validation.
			// IMPORTANT: Empty strings won't pass User::getCanonicalName but is still a valid realName
			if ($realName === false) {
				$realName = "";
			}
