...
abstract class QueryPage extends SpecialPage {
	 ...
	 */
	function isCached() {
		// Scott commented this out on 12/13/2013
		// so we just turn isExpensive on/off
		// for caching long query pages
		//global $wgMiserMode;
		//
...
abstract class WantedQueryPage extends QueryPage {
	...
	private function makeWlhLink( $title, $result ) {
		...
		// Reuben 1/28/2014: hack so that url doesn't end with .jpg, which nginx tries
		// to translate into a file access then barfs a 404
		return Linker::link( $wlh, $label, array(), array( 'target' => $title->getPrefixedText() ) );
	}
}
