...
class EditPage {
	...
	protected function runPostMergeFilters( Content $content, Status $status, User $user ) {
		...
		if ( !Hooks::run( 'EditFilterMergedContent',
			...
			// Reuben, upgrade 1.23: call this hook when there is a spam error
			Hooks::run( 'EditFilterMergedContentError',
				array( $this->mArticle->getContext(), $content, $status ) );
	...
	private function formatStatusErrors( Status $status ) {
		// wikiHow (Alberto), 2019-07-29 - simplify error HTML, to match pre-mwupgrade UI
		return $status->getWikiText( false, false, $this->context->getLanguage() );
...
ERROR;
	...
	public function showEditForm( $formCallback = null ) {
		...
		//CHANGED: Bebeth - don't want to show the parser info here
	...
	private function getSummaryInputAttributes( array $inputAttrs = null ) {
		...
		return ( is_array( $inputAttrs ) ? $inputAttrs : [] ) + [
			...
			'tabindex' => 4, //[sc] 7/2019 - gotta be bigger than our textbox tabindex
			'size' => 60,
			'spellcheck' => 'true',
		];
	}
	...
	protected function showStandardInputs( &$tabindex = 4 ) {
		...
		// JRS 12/2/13 reordering to match redesign
	...
	public static function getEditToolbar( $title = null ) {
		...
		Hooks::run( 'EditPageBeforeEditToolbar', array( &$toolarray ) );

		/****
		 * BS: This is a little hacky, but we put all the button definitions into a function called initToolbar,
		 * which the edit script will call once it's loaded and ready. We can refactor after the upgrade if we
		 * want.
		 */
		$script = 'function initToolbar(){';
		foreach ( $toolarray as $tool ) {
			...
			$params = array(
				$image = $wgStylePath . '/owl/images/' . $tool['image'],
			...
			$script .= Xml::encodeJsCall( 'WH.Editor.addButton', $params );
		}
	...
	public function spamPageWithContent( $match = false ) {
		...
		// Gershon, upgrade 1.21: return HTTP status code 409 for spam filtered content
		$out->setStatusCode(409);

		$out->addHTML( '<div id="spamprotected">' );
		$out->addWikiMsg( 'spamprotectiontext' );
		// Gershon, upgrade 1.21: preseve functionality of wikiHow 1.12 spam filter
		...
		// Gershon, upgrade 1.21: spam filter change
