...
class DifferenceEngine extends ContextSource {
	...
	private function showMissingRevision() {
		...
		// JRS 02/25/14 Set a 404 status code for this error since google doesn't like soft 404s
	...
	function showDiffPage( $diffOnly = false ) {
		...
		// WIKIHOW - added hook to add any extra output here
		wfRunHooks( 'DifferenceEngineShowDiffPage', array( &$out) );
		...
		if ( $this->mOldRev === false ) {
			...
			$samePage = true;
			$oldHeader = '';
			// WIKIHOW - added this hook to change the oldHeader
			wfRunHooks( 'DifferenceEngineOldHeaderNoOldRev', array( &$oldHeader) );
		} else {
			...
			// WIKIHOW - added this hook to change the oldHeader
			wfRunHooks( 'DifferenceEngineOldHeader', array( $this, &$oldHeader, $prevlink, $oldminor, $diffOnly, $ldel, $this->unhide ) );
		...
		// WIKIHOW - added this hook to change the newHeader
		wfRunHooks( 'DifferenceEngineNewHeader', array( $this, &$newHeader, $formattedRevisionTools, $nextlink, $rollback, $newminor, $diffOnly, $rdel, $this->unhide ) );
	 ...
	 * @return String
	 */
	// WIKIHOW - made this function public
	...
	function showDiff( $otitle, $ntitle, $notice = '' ) {
		// WIKIHOW - added hook here to effect the output
		wfRunHooks( 'DifferenceEngineShowDiff', array( &$this) );
	...
	public function getDiffBody() {
		...
		if ( $this->mOldRev === false || ( $this->mOldRev && $this->mNewRev
			...
			if ( !wfRunHooks( 'DifferenceEngineShowEmptyOldContent', array( &$this ) ) ) {
				wfProfileOut( __METHOD__ );
	 ...
	 *
	 * WIKIHOW - added "state" parameter to this function
	 *
	 * @param $rev Revision
	 * @param string $complete 'complete' to get the header wrapped depending
	 *        the visibility of the revision and a link to edit the page.
	 * @param state - WIKIHOW - the state of the revision
	 * @return String HTML fragment
	 */
	public function getRevisionHeader( Revision $rev, $complete = '', $state = '' ) {
		...
		// WIKIHOW - added hook to affect the revision header
		wfRunHooks("DifferenceEngineGetRevisionHeader", array($this, &$header, $state, $rev));
