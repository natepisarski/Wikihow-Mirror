...
abstract class QueryPage extends SpecialPage {
	...
	public function isCached() {
		// Wikihow/Scott: we turn on cached expensive special pages, regardless
		// of our MiserMode status
	...
	protected function showEmptyText() {
		...
		Hooks::run( 'wgQueryPagesNoResults', [ &$msg ] );
	...
	protected function executeLBFromResultWrapper( IResultWrapper $res, $ns = null ) {
		...
		$batch = new LinkBatch;
		foreach ( $res as $row ) {
			// Wikihow/Gaurang, June 2019: skip titles that are all underscores
			if ( preg_match('@^_+$@', $row->title) ) {
				continue;
			}
