...
abstract class WantedQueryPage extends QueryPage {
	...
	public function formatResult( $skin, $result ) {
		...
		if ( $title instanceof Title ) {
			...
			// Wikihow/Gaurang, June 2019: Hiding bad/ garbage entries on the
			// WantedPages. Return the line below, instead of empty string,
			// to show bad entries again.
			return '';
	...
	private function makeWlhLink( $title, $result ) {
		...
		// was: $wlh = SpecialPage::getTitleFor( 'Whatlinkshere', $title->getPrefixedText() );
		$label = $this->msg( 'nlinks' )->numParams( $result->value )->text();
		// Wikihow/Reuben, circa 2014: hack so that url doesn't end with .jpg, which nginx tries
		// to translate into a file access then barfs a 404
		return $this->getLinkRenderer()->makeLink( $wlh, $label, [], [ 'target' => $title->getPrefixedText() ] );
		// was: return $this->getLinkRenderer()->makeLink( $wlh, $label );
	}
