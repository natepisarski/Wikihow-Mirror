...
class ChangesList extends ContextSource {
	...
	public static function newFromContext( IContextSource $context, array $groups = [] ) {
		...
		if ( Hooks::run( 'FetchChangesList', [ $user, &$sk, &$list ] ) ) {
			// Trevor, always use old changes list
	...
	public function insertDateHeader( &$s, $rc_timestamp ) {
		...
		if ( $date != $this->lastdate ) {
			...
			//XXCHANGEDXX Bebeth: changed from h4 to h3
			$s .= Xml::element( 'h3', null, $date ) . "\n<ul class=\"special wh_block recentchanges_list\">";
			$this->lastdate = $date;
	...
	public function insertDiffHist( &$s, &$rc, $unpatrolled = null ) {
		...
		if (
			...
			// Reuben 1/15/14: Add some extra params to diff so that diff
			// can use the context with RecentChanges to move to next diff. Note: if
	...
	public function getArticleLink( &$rc, $unpatrolled, $watched ) {
		...
		// Reuben 1/28/2014: added this so that links to new articles can be
		// patrolled via RC
