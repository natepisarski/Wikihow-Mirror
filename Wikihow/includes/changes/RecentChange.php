...
class RecentChange {
	 ...
	 * @param bool $fromMaster true if and only if you need to get this rcid
	 *   from master to avoid any slave lag issues. Added by Reuben (during
	 *   Mediawiki upgrade 1.21) for GoodRevision.
	 * @return RecentChange
	 */
	public static function newFromId( $rcid, $fromMaster = false ) {
		return self::newFromConds( array( 'rc_id' => $rcid ), __METHOD__, array(), $fromMaster );
	}
	 ...
	 * @return RecentChange
	 */
	public static function newFromConds( $conds, $fname = __METHOD__, $options = array(), $fromMaster = false ) {
	...
	public function doMarkPatrolled( User $user, $auto = false, $force = false ) {
		...
		// own revisions
		if ( !$force && $user->getName() == $this->getAttribute( 'rc_user_text' )
