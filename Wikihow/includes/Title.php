...
class Title implements LinkTarget, IDBAccessObject {
	...
	public static function newFromTextThrow( $text, $defaultNamespace = NS_MAIN ) {
		...
		$t = new Title();
		$t->mDbkeyform = strtr( $filteredText, ' ', '-' );
	...
	public static function makeTitle( $ns, $title, $fragment = '', $interwiki = '' ) {
		...
		$t->mDbkeyform = strtr( $title, ' ', '-' );
		$t->mArticleID = ( $ns >= 0 ) ? -1 : 0;
		$t->mUrlform = wfUrlencode( $t->mDbkeyform );
		$t->mTextform = strtr( $title, '-', ' ' );
	...
	public function setFragment( $fragment ) {
		// Wikihow/Reuben, 2019: doing this '-' to ' ' conversion was breaking citations,
		// so I'm disabling this translation for now. If we end up running into other
		// URL fragment problems because of this change, we can revisit the fix
		// for this problem by making a change with the Cite extension to never
		// generate dashes. Look at the Cite::referencesFormatEntry() method to
		// fix that way instead.
		// was: $this->mFragment = strtr( substr( $fragment, 1 ), '-', ' ' );
		$this->mFragment = substr( $fragment, 1 );
	}
	...
	private function secureAndSplit() {
		...
		$this->mTextform = strtr( $this->mDbkeyform, '-', ' ' );
	...
	public function purgeSquid() {
		...
		Hooks::run( 'PreCDNPurge', array( $this, &$urls ) );
