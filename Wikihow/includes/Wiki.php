...
class MediaWiki {
	...
	private function parseTitle() {
		...
		if ( $request->getCheck( 'search' )
			&& strpos($title, 'Special:LSearch') !== 0
			&& $title != 'wikiHowTo'
			&& strpos($title, 'Special:Finner') !== 0
		) {
			// Compatibility with old search URLs which didn't use Special:Search
			// - Reuben, 1/9/14: we exclude our Special:LSearch from this fix
			// Just check for presence here, so blank requests still
			// show the search page when using ugly URLs (bug 8054).
			// - Jordan: further hack to deal with URLs that start with /wikiHowTo
			// (searches from home page large text box)
			// - Reuben: further hack to deal with URLs that look like this:
			// http://www.wikihow.com/Special:LSearch%3Fsearch%3D"/User:LindsieH"
			// - George: even furthererer hack to deal with 'index.php?title=Special:Finner'
			$ret = SpecialPage::getTitleFor( 'Search' );
		} elseif ( $curid ) {
			// URLs like this are generated by RC, because rc_title isn't always accurate
			$ret = Title::newFromID( $curid );
		} else {
			// JRS 1/24/14 Special case for wikiHowTo urls.  Use logged out
			if ($title == 'wikiHowTo') $title = 'Special:LSearch';
	...
	private function performRequest() {
		...
		// Redirect loops, no title in URL, $wgUsePathInfo URLs, and URLs with a variant
			...
			&& !count( $request->getValueNames( array( 'action', 'title', 'useformat' ) ) )
	...
	private function main() {
		...
		if (
			...
			if ( $request->wasPosted() ) {
				...
				$debugtext = "Redirected post from HTTP to HTTPS";
			}
			...
			$debugtext = $debugtext . " Redirected to HTTPS because forceHTTPs might be true.";
			wfDebugLog('redirects', $debugtext);
			$output->redirect( $redirUrl, '302' );
			$output->output();
			wfProfileOut( __METHOD__ );
			return;
		}
